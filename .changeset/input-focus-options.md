...
The Wellsourced showcase theme now uses them for its app's `.search-input:focus` look — fields rest on the alabaster `background` tint, and on focus snap to the clean surface behind an ochre (`secondary`) 1px border with a 10% ring of the same hue — via new `inputFocus` border and shadow primitives, tinted per palette in light and dark.
