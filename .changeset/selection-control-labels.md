...
`labelHidden` and `fullWidth` take `booleanAttribute`, as `uni-inline-button` and `uni-expand-area` already do, so `<uni-checkbox labelHidden>` reads as true rather than binding the falsy empty string. `fullWidth` on `uni-button`, `uni-input`, `uni-input-box`, `uni-select-input` and `uni-textarea` still lacks it and is inert as a bare attribute — untouched here, and worth doing as its own change.
