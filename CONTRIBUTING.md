...
- **React Stack:** Built via Vite. Keep `react`, `react-dom`, and `@uni-design-system/uni-core` marked as `external`. Complex helper hooks (e.g., `@dnd-kit`) are safely bundled *inside* the artifact.
...
1. **Generate a Changeset File:**
   ...
   pnpm exec changeset
   ```
2. **Select Packages:** Choose *only* the specific package(s) you modified using the spacebar. Do not worry about selecting all three; our global configuration will automatically scale the versions of the other locked dependencies behind the scenes.
...
5. **Commit & Push:** Commit your code changes *along with* the newly generated `.changeset/xxxx-xxxx.md` file and push your branch to GitHub.
