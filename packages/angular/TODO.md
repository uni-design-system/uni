...
- [x] ~~Remaining coverage: DataTable/SortHeader/Paginator against a `UniDatasource` fixture; MultiSelectDropdown filtering/selection.~~ Done 2026-09-06. **49 tests** across the three that had none, all driven through a real `UniRecordDatasource` rather than a stub, so they assert the round trip (click → datasource → re-rendered rows) instead of which method was called: sort-header 9 (the asc/desc/unsorted cycle, per-column isolation, numeric vs string ordering), paginator 17 (stepping, bounds and disabled states, `aria-current`, page-number truncation, the size and jump inputs, out-of-range input), data-table 23 (rows/cells, `cell` accessors, following the datasource through sort and page, `aria-sort` per column including `indet` → none, row click and keyboard reachability, loading, and detail rows staying `inert` until expanded). MultiSelectDropdown's filtering and selection were already covered by its own spec — that half of this item was stale.
...
- [ ] **`schematics-src/ng-add/ng-add.spec.ts` is excluded from the test run** (Angular 22 upgrade, 2026-09-06). `@angular-devkit/schematics` 22.x is CommonJS but depends on magic-string 1.0, which is ESM-only: Node loads that pairing natively via `require(esm)`, Vitest's module runner cannot, and no `server.deps` inline/external setting bridges it. The schematic's own logic is still covered by `transforms.spec.ts` (9 tests); what is lost is the 4 tests that drive the real `SchematicTestRunner`. Revisit when the devkit or Vitest fixes it, or run that one spec under plain Node.
