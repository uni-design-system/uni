...
describe('generate-runtime-theme', () => {
  ...
  it('stays under the payload budget that elision buys', () => {
    ...
    // block pushed a theme past a 20 KB ceiling, and `tag`'s selected and hover
    // rules — seven variants by three tones — pushed one past 24 KB); it is
    // sized to catch the icon set, which would more than double the payload,
    // not to freeze it.
    for (const { id, theme } of env.themes) {
      expect(JSON.stringify(theme).length, `${id} payload`).toBeLessThan(28_000);
    }
  });
